...
fetch("playlist.json")
  ...
  .then(data => {
    ...
    shufflePlaylist(); // si no querés aleatorio, comenta esta línea
    playlistLoaded = true;
...
function startCrossfade() {
  index = (index + 1) % playlist.length; // avanza siempre
  ...
  nextAudio.onloadedmetadata = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (t >= CROSSFADE_TIME) {
        ...
        audio = nextAudio;
        nextAudio = new Audio();
        scheduleCrossfade(); // sigue el ciclo
      }
    }, 50);
  };
}
